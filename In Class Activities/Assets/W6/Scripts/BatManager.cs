...
public class BatManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if (_bats[i] == null || _playerTransform == null) continue;
    ...
    private void CreateReactions(BatW6 bat)
    {
        ...
        int index = Random.Range(0, _messages.Length);
        SpawnReactionUI(bat, _messages[index]);
        // STEP 5 -------------------------------------------------------------
    }
    ...
    private void SpawnReactionUI(BatW6 bat, string message)
    {
        ...
        Transform parentTransform = layout != null ? layout.transform : bat.transform;

        if (_newTextTimers[index] >= _timeBetweenNewMessages)
        {
            _newTextTimers[index] = 0.0f;
            TMP_Text textObj = Instantiate(_reactionUiPrefab, parentTransform);
